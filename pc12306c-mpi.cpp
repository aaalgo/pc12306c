...
void monitor_throughput (vector<Client> const *clients, float cycle, bool *stop, int node) {
    Counters old;
    count_all(*clients, &old);
    std::chrono::milliseconds delta(int(cycle * 1000));
    auto next = std::chrono::system_clock::now() + delta;
    if (node <= 0) {    // do not print on MPI slaves
...
#pragma omp for
    ...
    string rank = "(" + boost::lexical_cast<string>(world_rank) + ") ";
#else
    int constexpr world_rank = -1;
    string rank;
#endif
    {
#ifdef MPI_VERSION
        MPI_Barrier(MPI_COMM_WORLD);
        cout << rank << "Starting clients on node " << world_rank << "..." << endl;

#else   // with MPI, graceful exit is difficult
        cout << rank << "Starting clients, interrupt with Ctrl+C..." << endl;
        ...
        future<void> ft = async(launch::async, monitor_throughput, &clients, cycle, &stop_monitor, world_rank);
        // wait for clients
        for (auto &client: clients) {
            client.join();
        }
        if (clients.empty()) {
            cout << rank << "No clients running, sleep 10s for testing..." << endl;
...
#ifdef MPI_VERSION
    // make each node report in serial
    for (int r = 0; r < world_size; ++r) {
        MPI_Barrier(MPI_COMM_WORLD);
        if (r == world_rank) {
#endif
    cout << rank << "send: " << cnts[CNT_SEND] << endl;
    cout << rank << "receive: " << cnts[CNT_RECV] << endl;
    cout << rank << "succeed: " << cnts[CNT_SUCC] << endl;
    cout << rank << "latency.mean: " << ba::mean(acc) * 1000 << " ms" << endl;
    cout << rank << "latency.std: " << sqrt(ba::variance(acc)) * 1000 << " ms" << endl;
    cout << rank << "latency.min: " << ba::min(acc) * 1000  << " ms" << endl;
    cout << rank << "latency.max: " << ba::max(acc) * 1000 << " ms" << endl;
    cout << rank << "latency.count: " << ba::count(acc) << endl;
    cout << rank << "fill.rate: " <<  1.0 * sold / tspace.total_sids() << "    (rate of inventory sold)" << endl;
    cout << rank << "fulfill.rate: " <<  1.0 * succ / asked << "     (rate of reqs satisfied)" << endl;
    cout << rank << "ooo.time.max: " << max_ood * 1000 << " ms    (big respIDs arrive before small ones)" << endl;
    cout << rank << "conflict1: " << conflict1 << "    (successful reservation containing failed requests)" << endl;
    cout << rank << "conflict2: " << conflict2 << "    (seats are sold twice)" << endl;
#ifdef MPI_VERSION
        }
    }
    MPI_Finalize();
